...
export default function App(): JSX.Element {
  ...
  useEffect(() => {
    setProjectName((previous) => (previous ? previous : inferProjectName(repoRoot, previous ?? '')));
  }, [repoRoot]);
  ...
  const handleStart = useCallback(async () => {
    ...
    const developerDefinitions = developersForm.map((developer, index) => ({
      name: developer.name?.trim() || DEFAULT_DEVELOPERS[index] || `Developer ${index + 1}`,
    ...
    const resolvedProjectName = inferProjectName(repoRoot, projectName) || 'Project';
    const resolvedBaseBranch = defaultBranch.trim() || 'main';
  ...
  const handleSelectRepo = useCallback(async () => {
    ...
    if (!resultSelection.canceled && resultSelection.path) {
      ...
      setProjectName((previous) => previous || inferProjectName(resultSelection.path ?? '', ''));
    }
  }, []);
      ...
      <main className="layout">
        <section className="control-panel">
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              <label htmlFor="default-branch">Base Branch</label>
              <input
                id="default-branch"
                type="text"
                value={defaultBranch}
          ...
          <div className="form-group">
            <label htmlFor="registry">Registry Path (optional)</label>
            <input
              id="registry"
              type="text"
              value={registryPath}
              onChange={(event) => setRegistryPath(event.target.value)}
              placeholder="~/.agent_hub/bigboss.db"
              disabled={isRunning}
            />
          </div>
